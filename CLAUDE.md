...
A Flask + Flask-RESTX JSON API for books, reviews, users, and search, deployed to Google App Engine standard. Despite living under a directory named `node/`, there is no JavaScript build — `index.html` is a single standalone browser client.
...
**`index.html`** is not served by Flask (Flask-RESTX owns `/` for Swagger); it's a static page deployed separately, with `API_URL` at the top of its script block. It has no register form — the first user must be created via Swagger or curl. CORS is open app-wide via `flask_cors`.
