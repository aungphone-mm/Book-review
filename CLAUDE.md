...
**Database** is `DATABASE_URL`, falling back to SQLite only for local dev. `config.py` rewrites the `postgres://` prefix Heroku-style providers hand out to `postgresql://`, which SQLAlchemy 1.4+ requires; Neon already issues `postgresql://`, so the rewrite is a no-op there. When `RENDER=true`, a missing `DATABASE_URL`, `SECRET_KEY`, or `JWT_SECRET_KEY` raises at startup rather than silently using a dev fallback.

`SQLALCHEMY_ENGINE_OPTIONS` sets `pool_pre_ping` and a 300s `pool_recycle`. This is not optional with Neon: its free tier suspends an idle compute and drops the connections, so a pooled socket that looked fine at checkout is dead by the next request. Without the pre-ping the first request after any quiet spell fails with "server closed the connection unexpectedly".
...
`render.yaml` is a Render blueprint: free web service, `init_db.py` in the build command, `gunicorn run:app` as the start command, and `SECRET_KEY`/`JWT_SECRET_KEY` generated by Render rather than stored here.

**The database is not a Render resource.** Render's free Postgres is deleted after ~30 days, so the blueprint declares no `databases:` block and `DATABASE_URL` is `sync: false` — Render prompts for it on apply, and the value is a Neon connection string. Neon's free tier has no expiry. This means the API and its data are on different providers; moving hosts is now a matter of re-pasting one environment variable.
